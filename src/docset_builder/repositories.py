...
def clone_or_update(name: str, pypi_info: PyPIInfo) -> Path:
    ...
    if not repository_dir.exists():
        clone_repository(repository_dir, pypi_info, logger)
    update_repository(repository_dir, logger)
    return repository_dir


def clone_repository(repository_dir: Path, pypi_info: PyPIInfo, _logger):
    ...
    #  command
    result = subprocess.run(["git", "clone", pypi_info.url], cwd=repository_dir.parent)
